...
- `decision-core`, `decision-capture`, `dependency-upgrade-decision`, `ci-rationale`, `handoff-context`, `pr-rationale`, `memory-promote`, 그리고 첫 번째 얇은 public workflow wrapper인 `prepare-handoff`를 포함한 시작용 skill template
...
- `prepare-handoff`를 첫 end-to-end 공개 workflow로 만들기 위해 로컬 context collector와 바로 쓸 수 있는 Markdown output을 추가
- `write-pr-rationale`에도 같은 직접 사용 가능한 레이어를 구현
