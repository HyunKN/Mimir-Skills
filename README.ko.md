...
`Mimir-Skills`는 AI coding agent를 위한 output-first workflow skill pack으로 전환 중입니다.
...
- `mimir_skills/` 아래의 shared CLI-friendly surface, 즉 `python -m mimir_skills list`, `python -m mimir_skills prepare-handoff`, `python -m mimir_skills write-pr-rationale`
- `adapters/codex/scripts/install_codex_skills.py`와 `docs/codex-local-install.ko.md`로 제공되는 첫 Codex 전용 로컬 install path
...
- shared CLI run과 future adapter가 함께 참조할 compact always-loaded rules baseline
- 공통 glossary
...
- 지금은 다른 agent-specific adapter를 추가하지 않고, shared CLI를 baseline으로 유지한 채 반복 사용에서 shared CLI 또는 documented support를 넘는 분명한 UX 이득이 보일 때만 두 번째 thin adapter를 다시 검토하기
- `prepare-handoff`는 현재 더 강한 workflow로 보고, stable public guidance로 고정하기 전에 진행 중인 non-clean task state에서 한 번 더 확인하기
- `write-pr-rationale`는 clean-state run에서 generic placeholder나 큰 수동 재작성에 의존하지 않고 더 강한 `why` context를 잡을 수 있을 때까지 active output shaping 상태로 유지하기
...
- [Always-Loaded Rules](docs/always-loaded-rules.ko.md)
- [Quick Start](docs/quick-start.ko.md)
- [Codex Local Install](docs/codex-local-install.ko.md)
- [Agent Support Levels](docs/agent-support-levels.ko.md)
