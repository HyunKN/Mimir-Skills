...
- `decision-core`, `decision-capture`, `dependency-upgrade-decision`, `ci-rationale`, `handoff-context`, `pr-rationale`, `memory-promote`, 그리고 첫 번째 public workflow wrapper인 `prepare-handoff`를 포함한 시작용 skill template
...
- 더 나은 output shape, 선택적 evidence 입력, 실제 사용 피드백을 통해 `prepare-handoff`를 더 다듬기
