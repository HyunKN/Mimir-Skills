...
- 다른 agent surface로 installation UX를 넓히기 전에, 새 shared CLI surface와 현재 Codex 로컬 adapter 경로에서 구조화된 실제 사용 피드백을 모으기
- 그 피드백을 바탕으로 `prepare-handoff`와 `write-pr-rationale`가 더 많은 output shaping이 필요한지, 아니면 공개 guidance로 고정할 만큼 충분한지 판단하기
