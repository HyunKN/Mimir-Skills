...
- 시작용 skill template와 `decision-core`, `decision-capture`, `dependency-upgrade-decision`, `ci-rationale`, `handoff-context`, `pr-rationale`
...
- 루트 README와 `docs/`에 대한 한국어 동반 문서, 그리고 공개 저장소 밖 로컬 workspace 메모의 한글 미러

다음 단계:

- 더 많은 사용 피드백 이후의 좁은 hook 또는 guardrail 결정
- dependency, CI, handoff, PR workflow를 넘어서는 추가 workflow-specific skill 확장
