...
- 실제로 무엇이 바뀌었는가
- 무엇이 근거였는가
- AI-assisted tooling이 이 변경에 어떤 영향을 줬고 그 guidance를 어떻게 검증했는가
- blast radius가 중요한 변경에서 어떤 승인이나 rollout 경계가 적용되었는가
- 무엇이 아직 위험하거나 불확실한가
- shared workflow나 운영 surface에 닿은 뒤 어떤 일이 있었는가
...
- 이 저장소는 artifact-level traceability와 governance context를 강화하지만, 완전한 deployment-control system은 아직 아닙니다
...
프로젝트에 skill을 한 줄로 설치합니다. 레포 클론 없이 가능합니다:

```bash
npx mimir-skills install --target <claude|codex|generic>
```

target에는 `claude`, `codex`, `generic` 중 하나를 넣으면 됩니다.
...
프로젝트 디렉토리에 `.claude/` 또는 `.codex/`가 있으면 target을 자동 감지합니다. `--target`으로 명시할 수도 있습니다.

설치 후에는 보통 아래 중 하나만 말하면 됩니다:
...
에이전트 라우팅이 약하거나 일관되지 않다면 더 명시적으로 말하면 됩니다:
...
자세한 옵션은 [Local Install (English)](docs/codex-local-install.md)을 참고합니다.
복사해서 바로 쓸 수 있는 프롬프트 모음은 [Prompt Macros (English)](docs/prompt-macros.md)를 참고합니다.
...
이미 Obsidian을 쓰고 있다면, `.ai/records/reports/` 아래의 linked Markdown artifact를 로컬 review workspace처럼 사용할 수 있습니다.
이건 optional이며 core workflow에 필수는 아닙니다.
...
- [Prompt Macros (English)](docs/prompt-macros.md)
- [Obsidian Workspace (English)](docs/obsidian-workspace.md)
...
- [Local Install (English)](docs/codex-local-install.md)
...
- canonical decision record는 위험한 shared-surface 변경에서 AI-assistance provenance, approval state, change-governance context를 선택적으로 담을 수 있습니다
