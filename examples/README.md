...
- `macos-flaky-quarantine/` shows a repeated flaky macOS visual failure where quarantining one spec in a shared workflow was chosen over retries or disabling the full job. It is the shared-workflow-change example for the `capture-ci-investigation` beta workflow.
