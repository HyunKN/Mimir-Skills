...
def render_handoff(
    ...
    *,
    ...
    lines.extend(render_file_section(status, diff))
...
def render_file_section(status: dict[str, Any], diff: dict[str, Any]) -> list[str]:
...
def default_risk_lines(status: dict[str, Any]) -> list[str]:
    lines: list[str] = []
    if status.get("porcelain"):
        lines.append("- The branch still contains uncommitted changes, so the next owner should verify the final intended state before sharing externally.")
    if not int_value(status.get("staged_count")) and not int_value(status.get("unstaged_count")) and not int_value(status.get("untracked_count")):
        lines.append("- No working-tree changes were detected; verify that any missing context already lives in commits or decision records.")
    return lines
...
def default_next_steps(status: dict[str, Any], diff: dict[str, Any]) -> list[str]:
    steps: list[str] = []
    if status.get("porcelain"):
        steps.append("- Review the listed changed files and confirm which updates should be committed before handoff.")
    if as_list(diff.get("all_changed_files")):
        steps.append("- Re-run any relevant validation and replace the placeholder validation section with concrete results before external sharing.")
    steps.append("- Add task-specific blockers, risks, and owner notes if this draft is going to another person or agent.")
    return steps
