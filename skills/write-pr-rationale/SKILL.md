...
1. Start with `scripts/generate_pr_rationale.py --repo <path>` when a quick reviewer-facing draft from local repository context is enough.
2. Use `scripts/collect_pr_context.py --repo <path> --output <context.json>` first when the branch snapshot should be inspected or reused before rendering the PR rationale.
