...
1. Start with `python -m decision_skills write-pr-rationale --repo <path>` when a quick reviewer-facing draft from local repository context is enough.
...
- Run `python -m decision_skills write-pr-rationale --repo <path>` when you need a directly usable reviewer-facing draft from the current repository state, including clean-state fallback from committed work when needed.
